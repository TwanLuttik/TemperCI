#!/usr/bin/env bash
# Guest agent must not report success when the runner starts a job and
# exits 0 without ever writing "completed with result:".
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
AGENT="${SCRIPT_DIR}/temperci-runner-agent.sh"
WORKDIR="$(mktemp -d /tmp/temperci-incomplete.XXXXXX)"
...
if [[ "${got}" != "98" ]]; then
  echo "FAIL: runner.exit=${got} want 98" >&2
  echo "--- agent.log ---" >&2
...
echo "remap_incomplete_test: OK (incomplete job → runner.exit=98)"
exit 0
